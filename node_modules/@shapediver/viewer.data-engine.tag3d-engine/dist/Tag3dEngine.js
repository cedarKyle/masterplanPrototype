...
let Tag3dEngine = class Tag3dEngine {
    ...
    loadContent(content) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (!content) {
                const error = new viewer_shared_services_1.ShapeDiverViewerDataProcessingError('Tag3dEngine.loadContent: Invalid content was provided to tag3d engine.');
                throw this._logger.handleError(viewer_shared_services_1.LOGGING_TOPIC.DATA_PROCESSING, `Tag3dEngine.loadContent`, error);
            }
            if (content.data && Array.isArray(content.data)) {
                for (let i = 0; i < content.data.length; i++) {
                    ...
                    for (let line of lineArray) {
                        ...
                        child.data.push(new viewer_shared_types_1.GeometryData(new viewer_shared_types_1.PrimitiveData(attributes, 4, null, new viewer_shared_types_1.MaterialStandardData({ color: tag3dInfo.color, metalness: 0, roughness: 1 }))));
                        node.children.push(child);
                    }
                }
            }
            else {
                const error = new viewer_shared_services_1.ShapeDiverViewerDataProcessingError('Tag3dEngine.loadContent: No tag3d data was provided to tag3d engine.');
                throw this._logger.handleError(viewer_shared_services_1.LOGGING_TOPIC.DATA_PROCESSING, `Tag3dEngine.loadContent`, error);
            }
            return node;
        });
