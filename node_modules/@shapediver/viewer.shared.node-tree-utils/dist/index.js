 ...
 * @returns
 */
const hasBoneInHierarchy = (bone, node) => {
    if (bone === node)
        return true;
 ...
 * @returns
 */
const findNodeWithOriginalId = (node, originalId) => {
 ...
 */
const cloneSkinData = (originalNode, clonedNode) => {
    // animation data has references to nodes, we need to adjust those
    const adjustAnimationData = (node) => {
        for (let j = 0; j < node.data.length; j++) {
            if (node.data[j] instanceof viewer_shared_types_1.AnimationData) {
                const anim = node.data[j];
...
exports.NodeTreeUtils = NodeTreeUtils;
